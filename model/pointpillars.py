...
class PointPillars(nn.Module):
    ...
    def forward(self, batched_pts, mode='test', batched_gt_bboxes=None, batched_gt_labels=None):
        batch_size = len(batched_pts) # 6
        # batched_pts: list[tensor] -> pillars: (p1 + p2 + ... + pb, num_points, c), # [40514, 32, 4]
        #                              coors_batch: (p1 + p2 + ... + pb, 1 + 3),  # [40514, 4]
        #                              num_points_per_pillar: (p1 + p2 + ... + pb, ), (b: batch size) # [40514]
        ...
        # coors_batch: (p1 + p2 + ... + pb, 1 + 3)
        # npoints_per_pillar: (p1 + p2 + ... + pb, )
        #                     -> pillar_features: (bs, out_channel, y_l, x_l) # [6, 64, 496, 432]
        pillar_features = self.pillar_encoder(pillars, coors_batch, npoints_per_pillar)
        # [B, C, H, W]


        # xs:  [(bs, 64, 248, 216), (bs, 128, 124, 108), (bs, 256, 62, 54)] # multi-scale feature
