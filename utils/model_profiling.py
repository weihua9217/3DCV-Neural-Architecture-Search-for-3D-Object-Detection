...
import uni3dm.uni3d as uni3d
import uni3dm.point_encoder as point_encoder
...
def module_profiling(self, input, output, num_forwards, verbose):
    ...
    if isinstance(self, nn.MultiheadAttention) or isinstance(input[0], list) or isinstance(output, list):
        pass
    elif isinstance(self, Voxelization):
        pass
    elif isinstance(self, hr.PillarEncoder):
        pass
    elif isinstance(self, hr.Head):
        pass
    elif isinstance(self, uni3d.Uni3D):
        pass
    elif isinstance(self, point_encoder.PointcloudEncoder):
        pass
    elif isinstance(self, point_encoder.Group):
        pass
    else:
        ins = input[0].size()
        outs = output.size()
    ...
    if isinstance(self, nn.Conv2d):
        ...
        # 因為一直報錯說找不到t所以暫時先註解掉，而且只有印出warning 不會影響計算。
        # if (not getattr(self, 'ignore_model_profiling', False) and
        #         self.n_macs == 0 and t not in ignore_zeros_t):

        # if (not getattr(self, 'ignore_model_profiling', False) and self.n_macs == 0):
        #     if udist.is_master():
...
def model_profiling(model,
    ...
    data = torch.rand(22512, 4)
    data2 = torch.rand(27,7)
    data3 = torch.rand(27)
