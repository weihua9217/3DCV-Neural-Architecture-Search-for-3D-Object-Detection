...
def run_one_epoch(epoch,
    ...
    if not FLAGS.use_hdfs and train:
    ...
    for batch_idx, data in enumerate(data_fetcher):    
        ...
        elif train == False and udist.is_master(): # single process for validation
    ...
    if FLAGS.dataset == 'kitti' and phase == 'val' and udist.is_master():
    ...
    torch.distributed.barrier()# 同步兩個 process
    ...
    for epoch in range(last_epoch + 1, FLAGS.num_epochs):
        ...
        if (epoch + 1) % FLAGS.eval_interval == 0:
            ...
            if FLAGS.prune_params['method'] is not None and FLAGS.prune_params['bn_prune_filter'] is not None:
                ...
                masks = prune.cal_mask_network_slimming_by_threshold(get_prune_weights(model_eval_wrapper), prune_threshold)  # get mask for all bn weights (depth-wise)
                FLAGS._bn_to_prune.add_info_list('mask', masks)
                
                flops_pruned, infos = prune.cal_pruned_flops(FLAGS._bn_to_prune)
                log_pruned_info(mc.unwrap_model(model_eval_wrapper), flops_pruned, infos, prune_threshold)
                
                if not FLAGS.distill:
                    if flops_pruned >= FLAGS.model_shrink_delta_flops or epoch == FLAGS.num_epochs - 1:
                        ema_only = (epoch == FLAGS.num_epochs - 1)
                        shrink_model(model_wrapper, ema, optimizer, FLAGS._bn_to_prune, prune_threshold, ema_only)
